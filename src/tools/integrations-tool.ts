...
export class ExaIntegrationsTool extends BaseTool {
  static readonly schema = ExaIntegrationsSchema;

  async execute(args: ExaIntegrationsArgs): Promise<string> {
    try {
      ...
      if (args.platform) {
        ...
        if (args.topic) {
          results = results.filter(doc => 
            doc.title.toLowerCase().includes(args.topic!.toLowerCase()) || 
  ...
  private filterByMethod(docs: DocumentationFile[], method: string): DocumentationFile[] {
    return docs.filter(doc => 
      doc.content.toLowerCase().includes(method.toLowerCase()) ||
      doc.title.toLowerCase().includes(method.toLowerCase())
    );
  }

  private filterByPlatform(searchResults: SearchResult[], platform: string): SearchResult[] {
    ...
    return searchResults.filter(result => {
      ...
      ) && (path.includes('integrations') || path.includes('sdks') || path.includes('reference'));
    });
  }

  private filterIntegrationResults(searchResults: SearchResult[]): SearchResult[] {
    return searchResults.filter(result => 
      result.path.startsWith('integrations/') || 
  ...
  private extractPlatform(path: string): string {
    if (path.includes('python-sdk')) return 'Python SDK';
    if (path.includes('typescript-sdk')) return 'TypeScript SDK';
    if (path.includes('langchain')) return 'LangChain';
    if (path.includes('llamaindex')) return 'LlamaIndex';
    if (path.includes('crewai')) return 'CrewAI';
    if (path.includes('openai')) return 'OpenAI';
    if (path.includes('vercel')) return 'Vercel';
    if (path.includes('ibm-watsonx')) return 'IBM WatsonX';
    if (path.includes('openrouter')) return 'OpenRouter';
    return 'Unknown';
  }

  private getIntegrationsOverview(): string {
    ...
    const platforms = integrations.filter(i => 
      i.path.includes('vercel') || 
