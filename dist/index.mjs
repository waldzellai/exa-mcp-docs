...
var BaseTool = class {
  ...
  loadDocsFromDirectory(dir, category = "") {
    ...
    for (const [filePath, doc] of this.docs) {
      ...
      if (matches.length > 0) {
        results.push({
          path: filePath,
          title: doc.title,
          content: this.truncateContent(doc.content, 2e3),
          relevanceScore,
          matches
        });
      }
...
${fuzzyMatches.slice(0, 5).map((p) => `- ${p}`).join("\n")}`,
  ...
  findSimilarPaths(target) {
    ...
    const similarities = allPaths.map((path2) => ({
      path: path2,
      score: this.calculateSimilarity(target, path2)
    }));
    return similarities.filter((s) => s.score > 0.3).sort((a, b) => b.score - a.score).map((s) => s.path);
  }
...
`;
      output += `${this.truncateContent(result.content, 2e3)}
...
// src/tools/docs-tool.ts
var ExaDocsSchema = external_exports.object({
  paths: external_exports.array(external_exports.string()).optional(),
  category: external_exports.enum(["api", "concepts", "guides", "admin", "reference"]).optional(),
  query: external_exports.array(external_exports.string()).optional()
});
var ExaDocsTool = class extends BaseTool {
  ...
  async execute(args) {
    try {
      ...
      if (args.category) {
        ...
        return this.formatResults(categoryResults.slice(0, 10));
      }
      if (args.query && args.query.length > 0) {
...
`;
  ...
  getConcepts() {
    return this.getDocumentationByCategory("reference").filter(
      (doc) => doc.path.includes("how-exa-search-works") || doc.path.includes("the-exa-index") || doc.path.includes("exas-capabilities-explained")
    );
  }
  getGettingStarted() {
...
// src/tools/examples-tool.ts
var ExaExamplesSchema = external_exports.object({
  example: external_exports.string().optional(),
  useCase: external_exports.enum(["research", "rag", "news", "analysis", "recruiting", "hallucination-detection"]).optional(),
  language: external_exports.enum(["python", "typescript", "javascript"]).optional(),
  query: external_exports.array(external_exports.string()).optional()
});
var ExaExamplesTool = class extends BaseTool {
...
// src/tools/integrations-tool.ts
var ExaIntegrationsSchema = external_exports.object({
  platform: external_exports.enum(["python-sdk", "js-sdk", "typescript-sdk", "langchain", "llamaindex", "crewai", "openai", "vercel", "ibm-watsonx", "openrouter"]).optional(),
  method: external_exports.string().optional(),
  topic: external_exports.string().optional(),
  query: external_exports.array(external_exports.string()).optional()
});
var ExaIntegrationsTool = class extends BaseTool {
      ...
      if (args.platform) {
        ...
        if (args.topic) {
          results = results.filter(
            (doc) => doc.title.toLowerCase().includes(args.topic.toLowerCase()) || doc.content.toLowerCase().includes(args.topic.toLowerCase())
          );
        }
        if (args.method) {
  ...
  getIntegrationsByPlatform(platform) {
    ...
    for (const path2 of paths) {
      const doc = this.docs.get(`${path2}.md`);
      if (doc) {
        results.push(doc);
      }
    }
    for (const [, doc] of this.docs) {
      if ((doc.category === "integrations" || doc.category === "reference") && doc.path.toLowerCase().includes(platform.toLowerCase()) && !results.some((r) => r.path === doc.path)) {
        results.push(doc);
      }
    }
    return results;
  ...
  filterByMethod(docs, method) {
    return docs.filter(
      (doc) => doc.content.toLowerCase().includes(method.toLowerCase()) || doc.title.toLowerCase().includes(method.toLowerCase())
    );
  }
  filterByPlatform(searchResults, platform) {
    const platformKeywords = platform.split("-");
    return searchResults.filter((result) => {
      const path2 = result.path.toLowerCase();
      const content = result.content.toLowerCase();
      return platformKeywords.some(
        (keyword) => path2.includes(keyword) || content.includes(keyword)
      ) && (path2.includes("integrations") || path2.includes("sdks") || path2.includes("reference"));
    });
  }
  filterIntegrationResults(searchResults) {
    return searchResults.filter(
      (result) => result.path.startsWith("integrations/") || result.path.startsWith("sdks/") || result.path.startsWith("reference/") && (result.path.includes("langchain") || result.path.includes("llamaindex") || result.path.includes("crewai") || result.path.includes("openai"))
    );
  }
  formatSearchResults(results) {
    if (results.length === 0) {
...
`;
  ...
  extractPlatform(path2) {
    if (path2.includes("python-sdk")) return "Python SDK";
    if (path2.includes("typescript-sdk")) return "TypeScript SDK";
    if (path2.includes("langchain")) return "LangChain";
    if (path2.includes("llamaindex")) return "LlamaIndex";
    if (path2.includes("crewai")) return "CrewAI";
    if (path2.includes("openai")) return "OpenAI";
    if (path2.includes("vercel")) return "Vercel";
    if (path2.includes("ibm-watsonx")) return "IBM WatsonX";
    if (path2.includes("openrouter")) return "OpenRouter";
    return "Unknown";
  }
...
`;
    ...
    const platforms = integrations.filter(
      (i) => i.path.includes("vercel") || i.path.includes("ibm-watsonx") || i.path.includes("openrouter")
    );
...
// src/tools/websets-tool.ts
var ExaWebsetsSchema = external_exports.object({
  feature: external_exports.enum(["monitors", "webhooks", "enrichments", "imports", "websets", "events", "searches", "items"]).optional(),
  operation: external_exports.enum(["create", "update", "delete", "list", "get", "cancel"]).optional(),
  includeExamples: external_exports.boolean().default(false)
});
var ExaWebsetsTool = class extends BaseTool {
  ...
  filterByOperation(docs, operation) {
    return docs.filter(
      (doc) => doc.path.includes(`${operation}-`) || doc.title.toLowerCase().includes(operation) || doc.content.toLowerCase().includes(`${operation} `)
    );
  }
...
`;
  ...
  extractFeature(path2) {
    if (path2.includes("/monitors/")) return "Monitors";
    if (path2.includes("/webhooks/")) return "Webhooks";
    if (path2.includes("/enrichments/")) return "Enrichments";
    if (path2.includes("/imports/")) return "Imports";
    if (path2.includes("/websets/")) return "Websets";
    if (path2.includes("/events/")) return "Events";
    if (path2.includes("/searches/")) return "Searches";
    if (path2.includes("/items/")) return "Items";
    if (path2.includes("/dashboard/")) return "Dashboard";
    return "General";
  }
  extractDescription(content) {
    const lines = content.split("\n");
...
// src/tools/changelog-tool.ts
var ExaChangelogSchema = external_exports.object({
  version: external_exports.string().optional(),
  changeType: external_exports.enum(["breaking", "feature", "fix", "deprecation", "enhancement"]).optional(),
  dateRange: external_exports.object({
    start: external_exports.string().optional(),
    end: external_exports.string().optional()
  }).optional()
});
var ExaChangelogTool = class extends BaseTool {
  static schema = ExaChangelogSchema;
  async execute(args) {
    try {
      if (args.version) {
...
`;
      const changeTypes2 = this.extractChangeType(change.content);
      if (changeTypes2.length > 0) {
        output += `**Types:** ${changeTypes2.join(", ")}
`;
      }
...
`;
    ...
    output += "\n## Migration Guides\n\n";
    const migrationGuides = changelog.filter(
      (c) => c.content.toLowerCase().includes("migration") || c.content.toLowerCase().includes("breaking")
    );
...
// src/index.ts
var docsTool = new ExaDocsTool();
var examplesTool = new ExaExamplesTool();
var integrationsTool = new ExaIntegrationsTool();
var websetsTool = new ExaWebsetsTool();
var changelogTool = new ExaChangelogTool();
async function main() {
  ...
  server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
      ...
      switch (name) {
        case "exaDocs":
          result = await docsTool.execute(toolArgs);
          break;
        case "exaExamples":
          result = await examplesTool.execute(toolArgs);
          break;
        case "exaIntegrations":
          result = await integrationsTool.execute(toolArgs);
          break;
        case "exaWebsets":
          result = await websetsTool.execute(toolArgs);
          break;
        case "exaChangelog":
          result = await changelogTool.execute(toolArgs);
          break;
        default:
