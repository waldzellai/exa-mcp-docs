...
export class JumpToStepTool {
     ...
     */
    async jumpToSpecificStep(tutorial, lessonName, stepName, state) {
        // Find target lesson
...
${availableSteps.map(name => `- ${name}`).join('\n')}`;
        ...
        const wrappedContent = await this.contentLoader.loadAndWrapContent(tutorial.name, lesson.name, step.name, step, lesson.title, stepNumber, totalSteps);
...
${wrappedContent}`;
     ...
     */
    async jumpToLesson(tutorial, lessonName, state) {
        // Find target lesson
        const lesson = tutorial.lessons.find(l => l.name === lessonName);
        if (!lesson) {
            const availableLessons = tutorial.lessons.map(l => l.name);
...
${availableLessons.map(name => `- ${name}`).join('\n')}`;
     ...
     */
    async jumpToTutorial(tutorial, state) {
        // Find first lesson
     ...
     */
    async markProgressToStep(tutorial, targetLesson, targetStep, state) {
     ...
     */
    generateJumpContext(tutorial, lesson, step) {
...
📊 Tutorial Progress: ${tutorialProgress}% (${stepPosition}/${totalSteps} steps)`;
     ...
     */
    async showAvailableTargets(tutorialName) {
        const state = await this.stateManager.loadState();
        if (!state) {
