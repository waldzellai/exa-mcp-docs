...
export class NextStepTool {
    ...
    async execute() {
        try {
            ...
            const nextLesson = updatedTutorial.lessons.find(lesson => lesson.steps.some(step => step.name === nextStep.name));
            if (!nextLesson) {
            ...
            const wrappedContent = await this.contentLoader.loadAndWrapContent(updatedTutorial.name, nextLesson.name, nextStep.name, nextStep, nextLesson.title, stepNumber, totalSteps);
            // Format completion message
            const completionMessage = TutorialFormatter.formatStepCompletion(currentStep.title, nextStep.title, updatedTutorial.title, stepNumber, totalSteps);
...
${wrappedContent}`;
     ...
     */
    async validateProgression(tutorial) {
